...
class CardealsspiderSpider(scrapy.Spider):
    ...
    def start_requests(self):
        with open('C:/Users/André/OneDrive/Documentos/Projetos/Webscraping/CarDealershipScraping/config.json','r', encoding='utf-8') as file:
            data = json.load(file)
    ...
    def parse_car_deals(self, response):
        ...
        for car_ad in car_ads:
            ...
            for name,xpath in ad_data.items():  
                if name=='ad_page':
                    page_url = car_ad.xpath(ad_data['ad_page']['url']).get()
                    #print("pageurl",page_url)
                    #l.add_value('page_url',page_url)
                    break
                if xpath != "":
                    l.add_xpath(name,xpath)
            #response.xpath(response.meta['config']['has_next_page']) 
            ...
            if('ad_page' in ad_data and ad_data['ad_page'] != None):
                ...
                yield l.load_item()
        if(response.meta['config']['has_next_page']!=None):
             next_page=response.xpath(response.meta['config']['has_next_page'])
        if len(car_ads) !=0 and (next_page is not None and len(next_page)!=0 ):  
